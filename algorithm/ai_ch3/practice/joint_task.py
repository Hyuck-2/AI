...
class JointSegmentationInsertionProblem(util.SearchProblem):
    ...
    def start_state(self):
        ...
        # state == (index, word that is going to be used at the moment)
    def is_end(self, state):
        return state[0] == len(self.query)

    def succ_and_cost(self, state):
        # should return a tuple with action, a new_state(=should include idx and word), cost in it
        results = list()
        idx, word_now = state
        for i in range(idx+1, len(self.query)+1):
            fills = self.possibleFills(self.query[idx:i])
            for fill in fills:
                results.append((fill, (i, fill), self.bigramCost(word_now, fill)))
        return results
